...
bool Item_func_set_collation::fix_length_and_dec(THD *thd)
{
  if (agg_arg_charsets_for_string_result(collation, args, 1))
    return true;
  // Pass "false" to avoid conflicting collations error
  Lex_explicit_charset_opt_collate cl(collation.collation, false);
  if (cl.merge_collate_or_error(m_set_collation))
    return true;
  collation.set(cl.charset_and_collation(), DERIVATION_EXPLICIT,
